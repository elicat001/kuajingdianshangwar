...
export default function BattleCardPage() {
  ...
  const salesChartOption = {
    ...
    yAxis: { type: 'value' },
    series: [{ type: 'line', data: skuMetrics.sales7d, smooth: true, areaStyle: { color: 'rgba(15,52,96,0.15)' }, itemStyle: { color: '#0f3460' } }],
  };

  const adsChartOption = {
    tooltip: { trigger: 'axis' },
    legend: { data: ['花费', 'ACOS%'] },
    xAxis: { type: 'category', data: days },
    yAxis: [{ type: 'value', name: '$' }, { type: 'value', name: '%' }],
    series: [
      { name: '花费', type: 'bar', data: skuMetrics.adsSpend7d, itemStyle: { color: '#16213e' } },
      { name: 'ACOS%', type: 'line', yAxisIndex: 1, data: skuMetrics.acos7d, itemStyle: { color: '#e94560' } },
    ],
  };
  ...
  return (
    <div>
      ...
      <Row gutter={[16, 16]}>
        ...
        <Col xs={8} lg={4}><Card bordered={false}><Statistic title="7日均销" value={Math.round(skuMetrics.units7d.reduce((a, b) => a + b, 0) / 7)} suffix="件" valueStyle={{ fontSize: 20 }} /></Card></Col>
